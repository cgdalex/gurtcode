...
void Drive::drive_stop(vex::brakeType mode){
  DriveL.stop(mode);
  DriveR.stop(mode);
  LeftFrontDrive.stop(mode);
  LeftBackDrive.stop(mode);
  RightBackDrive.stop(mode);
  RightFrontDrive.stop(mode);
}
...
void Drive::holonomic_drive_to_pose(float X_position, float Y_position, float angle, float drive_max_voltage, float heading_max_voltage, float drive_settle_error, float drive_settle_time, float drive_timeout, float drive_kp, float drive_ki, float drive_kd, float drive_starti, float heading_kp, float heading_ki, float heading_kd, float heading_starti){
  ...
  while( !(drivePID.is_settled() && turnPID.is_settled()) ){
    ...
    LeftFrontDrive.spin(fwd, drive_output*cos(to_rad(get_absolute_heading()) + heading_error - M_PI/4) + turn_output, volt);
    LeftBackDrive.spin(fwd, drive_output*cos(-to_rad(get_absolute_heading()) - heading_error + 3*M_PI/4) + turn_output, volt);
    RightBackDrive.spin(fwd, drive_output*cos(to_rad(get_absolute_heading()) + heading_error - M_PI/4) - turn_output, volt);
    RightFrontDrive.spin(fwd, drive_output*cos(-to_rad(get_absolute_heading()) - heading_error + 3*M_PI/4) - turn_output, volt);
    task::sleep(10);
  }
}
...
void Drive::control_arcade(){
  ...
  DriveL.spin(fwd, to_volt((throttle+turn)/2), volt);
  DriveR.spin(fwd, to_volt((throttle-turn)/2), volt);
}
...
void Drive::control_holonomic(){
  ...
  LeftFrontDrive.spin(fwd, to_volt(throttle+turn+strafe), volt);
  LeftBackDrive.spin(fwd, to_volt(throttle+turn-strafe), volt);
  RightBackDrive.spin(fwd, to_volt(throttle-turn+strafe), volt);
  RightFrontDrive.spin(fwd, to_volt(throttle-turn-strafe), volt);
...
void Drive::drive_with_pursuit_path(const std::vector<Waypoint>& path) {
  drive_with_pursuit_path(path, /*setback=*/0.0f, drive_max_voltage, heading_max_voltage, drive_settle_error, drive_settle_time, drive_timeout, drive_kp, drive_ki, drive_kd, drive_starti, heading_kp, heading_ki, heading_kd, heading_starti);
}

void Drive::drive_with_pursuit_path(const std::vector<Waypoint>& path, float setback) {
  drive_with_pursuit_path(path, setback, drive_max_voltage, heading_max_voltage, drive_settle_error, drive_settle_time, drive_timeout, drive_kp, drive_ki, drive_kd, drive_starti, heading_kp, heading_ki, heading_kd, heading_starti);
}

void Drive::drive_with_pursuit_path(const std::vector<Waypoint>& path, float setback, float drive_max_voltage, float heading_max_voltage) {
  drive_with_pursuit_path(path, setback, drive_max_voltage, heading_max_voltage, drive_settle_error, drive_settle_time, drive_timeout, drive_kp, drive_ki, drive_kd, drive_starti, heading_kp, heading_ki, heading_kd, heading_starti);
}

void Drive::drive_with_pursuit_path(const std::vector<Waypoint>& path, float setback, float drive_max_voltage, float heading_max_voltage, float drive_settle_error, float drive_settle_time, float drive_timeout) {
  drive_with_pursuit_path(path, setback, drive_max_voltage, heading_max_voltage, drive_settle_error, drive_settle_time, drive_timeout, drive_kp, drive_ki, drive_kd, drive_starti, heading_kp, heading_ki, heading_kd, heading_starti);
}
